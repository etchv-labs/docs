...
Install from the public SDK repository (not yet published on PyPI):
...
Use the document methods for [PDF watermarking](/docs/api/documents), and the video methods for [supported MP4/MOV videos](/docs/api/videos). PDF preserves selectable text and vectors. Video operations use durable jobs and charge one credit per started minute. The existing `image` result field contains the native file bytes.

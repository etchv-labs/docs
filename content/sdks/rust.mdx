Use the [Etchv Rust SDK](https://github.com/etchv-labs/rust-sdk) with stable Rust (edition 2024).
...
let job = client.submit_embed("documents", &pdf_bytes,
    ...
    etchv::Options { filename: Some("document.pdf".into()),
        idempotency_key: Some("delivery_001".into()), ..Default::default() }, Some(&webhook_id))?;
...
let delivery = client.get_storage_delivery(job["storage_delivery_id"].as_str().unwrap())?;
```
